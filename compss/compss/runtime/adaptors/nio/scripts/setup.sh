#!/bin/bash
  ...
  setup_environment(){
    ...
    # TODO: set this in a general way
    export FUSEKI_HOME=$HOME/projects/semantic_compss/resources/jena-fuseki
    export FUSEKI_PORT="303${hostId}"
    # Start Worker Fuseki
    nohup ${FUSEKI_HOME}/fuseki-server --update --port="${FUSEKI_PORT}" --loc=${workingDir}/data /compssW > ${workingDir}/worker_fuseki.out 2> ${workingDir}/worker_fuseki.err &
    fuseki_pid=$!
    endCode=$?
    echo "Fuseki PID: ${fuseki_pid}"
    echo "Fuseki Home: ${FUSEKI_HOME}"
    echo "Fuseki Port: ${FUSEKI_HOME}"
    echo "endCode: ${endCode}"
    export FUSEKI_PID=${fuseki_pid}
    if [ $endCode -eq 0 ]; then
        echo "Fuseki started successfully and listening on port ${FUSEKI_PORT}"
    else
        echo "[ERROR] Fuseki could not be started."
    fi
  ...
  clean_env() {
    ...
    echo "Killing fuseki process $FUSEKI_PID"
    kill -9 $FUSEKI_PID
    #rm -rf ${workingDir}
  }
